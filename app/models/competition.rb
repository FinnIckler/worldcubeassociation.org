  ...
  def info_for(user)
    ...
    if self.in_progress? && !self.cancelled?
      if self.use_wca_live_for_scoretaking
        #info[:in_progress] = I18n.t('competitions.messages.in_progress_at_wca_live_html', link_here: Rails.application.routes.url_helpers.live_schedule_path(id)).html_safe
      else
